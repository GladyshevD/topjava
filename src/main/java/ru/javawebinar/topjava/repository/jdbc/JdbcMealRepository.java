...
public class JdbcMealRepository implements MealRepository {
        ...
        if (meal.isNew()) {
            ...
            meal.setId(newKey.intValue());
                ...
                "UPDATE meals SET description=:description, datetime=:datetime, " +
                        "calories=:calories, user_id=:user_id WHERE id=:id", mapSqlParameterSource) == 0) {
            return null;
        }
        return meal;
    }
    ...
    public List<Meal> getBetween(LocalDateTime startDate, LocalDateTime endDate, int userId) {
        return jdbcTemplate.query("SELECT * FROM meals WHERE user_id = ? AND datetime >= ? AND datetime <= ? ORDER BY datetime DESC",
